...
public class ArenaTask extends DelayedTask {
    ...
    private static final DTOPoint CHALLENGES_LEFT_BOTTOM_RIGHT = new DTOPoint(455, 986);
    ...
    // Battle controls
    private static final DTOPoint QUICK_DEPLOY_BUTTON = new DTOPoint(180, 1200);
    ...
    /**
     * ArenaTask navigates into arena menus and runs battles.
    ...
    private boolean readInitialAttempts() {
        ...
        DTOTesseractSettings strictSettings = DTOTesseractSettings.builder()
                .setRemoveBackground(true)
        ...
        // Second attempt: Relaxed settings (no color filter) if strict failed
        // This handles cases like number "7" where color detection might fail
        if (attemptsRead == null) {
            logWarning("Strict OCR failed for attempts count. Retrying with relaxed settings...");

            DTOTesseractSettings relaxedSettings = DTOTesseractSettings.builder()
                    .setRemoveBackground(true)
                    .setAllowedChars("0123456789")
                    .build();
        ...
        if (attemptsRead == null) {
            logWarning(
                    "Failed to read initial attempts via OCR (both strict and relaxed attempts failed). Defaulting to 5.");
            this.attempts = 5;
            return true;
        }
