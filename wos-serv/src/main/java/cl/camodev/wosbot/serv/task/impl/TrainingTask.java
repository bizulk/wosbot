...
public class TrainingTask extends DelayedTask {
    ...
    private void executeLimitedTrainingForAppointment(QueueInfo queue) {
        LocalDateTime now = LocalDateTime.now();
        // Calculate exact time until appointment (OCR retries provide sufficient safety
        // margin)
    ...
    private int findMaxAvailableTroopLevel(TroopType troopType) {
        ...
        for (EnumTemplates template : templates) {
            ...
            if (troop.isFound()) {
                ...
                sleepTask(200); // Wait for scroll animation
                emuManager.captureScreenshotViaADB(EMULATOR_NUMBER);
            }
        }
    ...
    private boolean attemptSingleTroopPromotion(EnumTemplates template) {
        ...
        if (troop.isFound()) {
            ...
            sleepTask(300); // Wait for details
            ...
            logDebug("Template not found: " + template.name());
    ...
    private boolean executePromotion(EnumTemplates template, DTOImageSearchResult promoteButton) {
        ...
        sleepTask(300); // Wait for promotion to process
    ...
    private void selectHighestTroopLevel(TroopType troopType) {
        ...
        for (EnumTemplates template : templates) {
            ...
            if (troop.isFound()) {
                tapPoint(troop.getPoint());
                sleepTask(250); // Wait for selection
     ...
     * Note: Assumes highest troop level is already selected before calling this
     * method.
     */
    private void clickTrainButton() {
        ...
        if (trainButton.isFound()) {
            ...
            emuManager.captureScreenshotViaADB(EMULATOR_NUMBER);
            DTOImageSearchResult replenishAll = templateSearchHelper.searchTemplate(
                    TRAINING_REPLENISH_ALL,
                    SearchConfigConstants.DEFAULT_SINGLE);

            if (replenishAll.isFound()) {
                logInfo("Filling resources.");
                tapRandomPoint(replenishAll.getPoint(), replenishAll.getPoint(), 1, 300);
                sleepTask(300);
                DTOPoint replenishConfirm = new DTOPoint(442, 1064);
     ...
     * Strategy:
     * <ul>
     * <li>If completion times available: Schedule for earliest to enable continuous
     * training</li>
     * <li>If earliest is very close to appointment (within threshold): Wait for
     * appointment instead</li>
    ...
    private void rescheduleToEarliestCompletion(List<LocalDateTime> completionTimes) {
        ...
        // If ministry appointment enabled and earliest completion is very close to
        // appointment,
        // wait for appointment to maximize the bonus window instead of training more
        // limited troops
        if (ministryAppointmentEnabled && appointmentTime != null) {
