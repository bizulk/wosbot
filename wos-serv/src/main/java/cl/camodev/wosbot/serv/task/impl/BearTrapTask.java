...
public class BearTrapTask extends DelayedTask {
    ...
    // ========== OCR Regions ==========
    private static final DTOPoint MARCH_TIME_OCR_TL = new DTOPoint(504, 1134);
    private static final DTOPoint MARCH_TIME_OCR_BR = new DTOPoint(622, 1162);
    ...
    /**
     * BearTrapTask owns the emulator screen exclusively for up to 30+ minutes
     * (preparation + active trap phases). Allowing injection rules to fire during
     * that window risks corrupting the navigation state.
     * 
     * @return {@code false} — injections are suppressed for the entire task
     *         duration
     */
    @Override
    ...
    /**
     * Parses the join flag configuration (comma-separated string) into a sorted
     * list.
     * 
     * <p>
     * Examples:
    ...
    protected void execute() {
        ...
        try {
            ...
            if (isVisuallyTriggered) {
                logInfo("Task was Visually Triggered! Bypassing scheduled configuration and forcing 30-minute Active execution.");
                LocalDateTime now = LocalDateTime.now(ZoneId.of("UTC"));
                timing = new TrapTiming(now, now, now.plusMinutes(TRAP_DURATION_MINUTES));
            } else {
                timing = calculateTrapTiming();
    ...
    private boolean verifyExecutionWindow() {
        // Reset the flag for this execution attempt
        isVisuallyTriggered = false;

        // Check visual indicator first - triggers from Global Monitor should always
        // proceed
        try {
            DTOImageSearchResult result = emuManager.searchTemplate(
                    ...
                    90);
            if (result.isFound()) {
                logInfo("Confirmed: Bear Hunt is VISUALLY ACTIVE. Overriding time window check.");
                isVisuallyTriggered = true;
                return true;
            }
        } catch (Exception e) {
            logWarning("Visual check failed in verifyExecutionWindow: " + e.getMessage());
        }
    ...
    private void executeTrapActivePhase(LocalDateTime trapEndTime) {
        ...
        while (now.isBefore(trapEndTime)) {
            checkPreemption(); // honour preemption at every iteration boundary
            iterationCount++;
    ...
    private int checkFreeMarches() {
        checkPreemption(); // honour preemption before heavy OCR block
        ...
        checkPreemption(); // honour preemption between the two OCR calls
    ...
    private long startOwnRally() {
        ...
        if (marchSeconds == 0) {
            logError("Could not read march time from screen, defaulting to 30 seconds");
            marchSeconds = 30; // fallback march time duration
        }
